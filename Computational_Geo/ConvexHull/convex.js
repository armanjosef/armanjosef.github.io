...
var wasmBinaryFile = 'convex.wasm';
...
function drawLine(p,q){ var ctx = document.getElementById("canvas").getContext("2d"); ctx.beginPath(); ctx.lineWidth = 1; ctx.strokeStyle = "blue"; ctx.moveTo(p.x, p.y); ctx.lineTo(q.x, q.y); ctx.stroke(); }
function drawPoint(p,q){ var ctx = document.getElementById("canvas").getContext("2d"); console.log(ctx); console.log("drawPoint"); ctx.beginPath(); ctx.arc(p.x + 1.5, p.y + 1.5, 3, 0, Math.PI * 2, true); ctx.fillStyle = "black"; ctx.fill(); ctx.arc(q.x + 1.5, q.y + 1.5, 3, 0, Math.PI * 2, true); ctx.fillStyle = "black"; ctx.fill(); }
function displayLog(){ console.log("slowConvex() called"); }



// STATICTOP = STATIC_BASE + 23488;
